...
class EmployeeViewSet(viewsets.ModelViewSet):
    ...
    Query optimizations:
    - Annotates attendance counts directly on the queryset (no N+1)
    - select_related where FKs exist (future-proofing)
    """
    ...
    def list(self, request, *args, **kwargs):
        """Paginated employee list — hits DB directly, no cache layer."""
        return super().list(request, *args, **kwargs)
    ...
    def dashboard_stats(self, request):
        """
        Aggregated stats for the dashboard.
        Cached in LocMemCache for 30 seconds to reduce DB load on repeated
        dashboard refreshes without requiring Redis.
        """
        from attendance.models import Attendance
        from django.utils import timezone
    ...
    @action(detail=False, methods=["get"], url_path="attendance-overview")
    def attendance_overview(self, request):
        """
        Returns ALL employees with their attendance status for a given date.
        Employees with no record get status = "Not Marked".
        One query using a LEFT OUTER JOIN via subquery annotation.
        """
        from attendance.models import Attendance
        from django.db.models import OuterRef, Subquery, CharField, Value
        from django.db.models.functions import Coalesce
        from django.utils.dateparse import parse_date
        from .serializers import AttendanceOverviewSerializer
        ...
        attendance_status = Attendance.objects.filter(
            employee=OuterRef("pk"), date=target_date
        ).values("status")[:1]

        attendance_id = Attendance.objects.filter(
            employee=OuterRef("pk"), date=target_date
        ).values("id")[:1]

        attendance_notes = Attendance.objects.filter(
            employee=OuterRef("pk"), date=target_date
        ).values("notes")[:1]
        ...
        queryset = (
            ...
            .annotate(
                day_status=Coalesce(
                    Subquery(attendance_status, output_field=CharField()),
                    Value("Not Marked"),
                ),
                attendance_record_id=Subquery(attendance_id),
                attendance_notes=Coalesce(
                    Subquery(attendance_notes, output_field=CharField()),
                    Value(""),
                ),
            )
            .order_by("full_name")
        ...
        if filter_status:
            queryset = queryset.filter(day_status=filter_status)
        ...
        page = self.paginate_queryset(queryset)
        if page is not None:
            serializer = AttendanceOverviewSerializer(
                page, many=True, context={"date": target_date}
            )
            return self.get_paginated_response(serializer.data)

        serializer = AttendanceOverviewSerializer(
            queryset, many=True, context={"date": target_date}
        )
        return Response(serializer.data)
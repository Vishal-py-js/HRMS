...
"""
Cache helpers — no Redis dependency.
Uses Django's built-in LocMemCache (in-memory, per-process).

Note: LocMemCache does not support delete_pattern so all cache
invalidation uses cache.clear() — safe since the cache is small
and in-memory, making clears near-instant.
"""
from django.core.cache import cache
import logging
...
# Cache TTLs (seconds)
EMPLOYEE_LIST_TTL   = 30
EMPLOYEE_DETAIL_TTL = 60
ATTENDANCE_LIST_TTL = 20
DASHBOARD_STATS_TTL = 30
...
class CacheKeys:
    ...
    @staticmethod
    def employee_list(page, page_size, search="", department=""):
        return f"employees:list:p{page}:ps{page_size}:s{search}:d{department}"

    @staticmethod
    def employee_detail(employee_id):
        return f"employees:detail:{employee_id}"

    @staticmethod
    def attendance_list(employee_id, page, page_size, date="", status=""):
        return f"attendance:list:{employee_id}:p{page}:ps{page_size}:dt{date}:st{status}"
...
def invalidate_employee_caches():
    """
    Clear all caches after any employee mutation.
    LocMemCache has no pattern deletion so we clear the whole cache.
    It is in-memory only so this is instant with no side effects.
    """
    try:
        cache.clear()
        logger.debug("Cache cleared after employee mutation")
    except Exception as exc:
        logger.warning("Cache clear failed: %s", exc)


def invalidate_attendance_caches(employee_id):
    """Clear all caches after any attendance mutation."""
    try:
        cache.clear()
        logger.debug("Cache cleared after attendance mutation for %s", employee_id)
    except Exception as exc:
        logger.warning("Cache clear failed: %s", exc)